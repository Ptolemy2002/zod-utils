...
# prefixZodIssuePath
```typescript
function prefixZodIssuePath<I extends InterpretableZodIssue>(issue: I, prefix: ZodPath): I
```
...
- `prefix` (`ZodPath`): A key or array of keys to prepend to the issue's path. Non-array values are wrapped in a single-element array. Empty strings are filtered out of the final path.
